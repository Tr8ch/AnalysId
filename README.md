...
1) Получение документа для подписывания его NCAlayer-ом (так работает *idocs*) 
...
1) Подписываем документ с помощью NCAlayer-а

!***idocs подписывает непосредственно сам хэш соданного вами документа, то есть по закону элктронным документом считается этот самый хэш файл***!

- И так чтобы подписать документ нам нужно создать подключение(connect) к webSocket-у(имеено так *idocs* отправляет запрос на подписание NCAlayer-у)
- Можете попробывать через приложение *webSockat* или *PostMan*
...
P.S.: Полученный `.xml` файл это тот же файл что *idocs* отправляет нам при скачивания оригинала и подписей с их сервиса в папках подписантов(папка с `.xml` подписью) 
